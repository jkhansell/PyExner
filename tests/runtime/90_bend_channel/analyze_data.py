    ...
    for i, name in enumerate(points.keys()):
        ...
        if j == 0:
            if i == 0:
                ...
                ax.plot(
                    time_array_cut,
                    ds_point.h.values-0.33, #there is nothing but water in the reservoir
                    linewidth=1.5,
                    label="Simulación",
                    color='blue' 
                )
...
for j, data in enumerate(datasets):
    ...
    for i, (name, x_val) in enumerate(lines.items()):
        ...
        # bed profile along y
        z_sim = data_cut.z_b.values[200, :, ix]
